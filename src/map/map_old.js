...
function load_map_components() {
  d3.json("../assets/data/cities.json", function(error, data) {
    cities = data;
    cities_circles = cities_container
      .selectAll("circle")
      .data(cities)
      .enter()
      .append("circle")
      ...
      .attr("cx", function(d) {
        return projection([d.coordinates.x, d.coordinates.y])[0];
      })
      .attr("cy", function(d) {
        return projection([d.coordinates.x, d.coordinates.y])[1];
      })
      .attr("r", 2)
      .attr("fill", "black")
    ...
    function handleMouseOverCircle(d) {
      tooltip.transition("tooltip-1").style("opacity", 0.9);
      tooltip
        .html("<b>" + d.name + "</b>")
  ...
  // Mock Data loading!
  d3.json("../assets/data/mock-data-v7.json", function(error, data) {
    mock_data = data.data;
    ...
    project_circles
      .attr("id", function(d) {
        return "project-" + d.survey_answers.project_id;
      })
      .attr("cx", function(d) {
        var c = project_coordinates[d.survey_answers.location];
        return projection([c.x, c.y])[0];
      })
      .attr("cy", function(d) {
        var c = project_coordinates[d.survey_answers.location];
        return projection([c.x, c.y])[1];
      })
      .attr("r", function(d) {
        var city = d.survey_answers.location;
        var a = project_count_city[city];
    ...
    function handleClick(d) {
      ...
      if (d.survey_answers.group == 0) {
        ...
        document.getElementById("project_title").innerHTML =
          d.survey_answers.project_title;
        document.getElementById("project_subtitle").innerHTML =
          d.survey_answers.project_type;
        document.getElementById("project_leader").innerHTML =
          d.survey_answers.project_organization;
        document.getElementById("project_contact").innerHTML =
          "<a href='mailto:" +
          d.survey_answers.project_manager_email +
          "'>" +
          d.survey_answers.project_manager +
          "</a>";

        document.getElementById("budget").innerHTML =
          d.survey_answers.budget.funded;
        document.getElementById("proj-pic").innerHTML =
          "<img src='resources/thumbnails/" + pic_name[proj_id] + "' />";

        var dates =
          d.survey_answers.dates.start + " - " + d.survey_answers.dates.end;
        ...
        document.getElementById("card_descriptive_text").innerHTML =
          d.survey_answers.description;
        if (d.survey_answers.website != undefined) {
          document.getElementById("card_webesite_link").innerHTML =
            d.survey_answers.website;
        }
      }

      if (d.survey_answers.group == 1) {
        var tx =
          "<b>Projects</b><button onclick=\"console.log(5);\">hello</button><br><ol type='1'>";

        for (var i = 0; i < mock_data.length; i++) {
          if (mock_data[i].survey_answers.location.match(city)) {
            tx +=
              '<li onclick="crash();">' +
              mock_data[i].survey_answers.project_title +
              "</li>";
          }
        }
        tx += "</ol>";
    ...
    function handleMouseOverCircle(d) {
      var tx = "";
      if (
        project_count_city[d.survey_answers.location] > 1 &&
        d.survey_answers.group == 1
      ) {
        tx =
          "<b>" + d.survey_answers.location + "</b><br>Click to see projects";
      } else {
        tx =
          "<b>" +
          d.survey_answers.location +
          "</b>\n<br>" +
          d.survey_answers.project_title;
      }
  ...
  var q = d3.queue();
  q.defer(d3.json, "../assets/data/europe.topo.json");
  q.defer(d3.json, "../assets/data/sweden.topo.json");
...
function show_project(proj_id) {
  ...
  document.getElementById("project_title").innerHTML =
    d.survey_answers.project_title +
    '<span onclick="hide_text(0)"style="float:right;cursor:pointer;"><h1>X</h1></span>';
  document.getElementById("project_subtitle").innerHTML =
    d.survey_answers.project_type;
  document.getElementById("project_leader").innerHTML =
    d.survey_answers.project_organization;
...
function toggle_about(a) {
  if (a == 0) {
    d3.select(".about_box").style("display", "initial");
    d3.select(".inner_content").classed("blurredElement", true);
  } else if (a == 1) {
    d3.select(".about_box").style("display", "none");
    d3.select(".inner_content").classed("blurredElement", false);
  }
  about_showed = a;
}
...
window.addEventListener("click", function(e) {
  if (about_showed == 1) {
    if (document.getElementById("about_box").contains(e.target)) {
      ...
      if (
        !document.getElementById("about_tog").contains(e.target) &&
        about_showed == 0
      ) {
        toggle_about(1);
      }
